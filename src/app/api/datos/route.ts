...
const getDefault = async () => {
  ...
  // Si no hay datos cargar desde JSON inicial
  const fs = require('fs')
  const path = require('path')
  const raw = fs.readFileSync(path.join(process.cwd(), 'src/data/catalogo.json'), 'utf-8')
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (action === 'addEspecie') {
      const fam = data.find((f: any) => f.id === payload.familia)
      if (fam) {
        fam.e.push({n:payload.n,p:payload.p||0,s:payload.s||0})
        await redis.set(KEY, data)
      }
      return NextResponse.json({ ok: true })
    }
    if (action === 'deleteEspecie') {
      const fam = data.find((f: any) => f.id === payload.familia)
      if (fam) {
        fam.e = fam.e.filter((e: any) => e.n !== payload.n)
        await redis.set(KEY, data)
      }
      return NextResponse.json({ ok: true })
    }
    ...
    if (action === 'updateFamilia') {
      ...
      if (idx >= 0) {
        data[idx] = {...data[idx], id: payload.id, nm: payload.nm}
        await redis.set(KEY, data)
      }
      return NextResponse.json({ ok: true })
    }
