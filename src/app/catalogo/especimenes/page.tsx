...
function PopupHeader({title,onClose,foto,nombre}:{title:string,onClose:()=>void,foto?:string,nombre?:string}) {
  ...
  const vistasLB = (e:E) => {
    const v = []
    if(e.foto) v.push({label:'Frente',url:e.foto,tipo:'img'})
    else v.push({label:'Frente',url:'',tipo:'img'})
    if(e.fotoLado) v.push({label:'Lado',url:e.fotoLado,tipo:'img'})
    else v.push({label:'Lado',url:'',tipo:'img'})
    if(e.fotoReverso) v.push({label:'Reverso',url:e.fotoReverso,tipo:'img'})
    else v.push({label:'Reverso',url:'',tipo:'img'})
    if(e.video) v.push({label:'Video',url:e.video,tipo:'video'})
    else v.push({label:'Video',url:'',tipo:'video'})
    return v
  }

  const abrirLB = (e:E) => { setLbEsp(e); setLbVista(0); setLbOpen(true) }
  ...
  return (
    <div>
      ...
      {/* LIGHTBOX */}
      <div className={`lb-overlay${lbOpen?' lb-open':''}`} onClick={(e)=>{if((e.target as HTMLElement).classList.contains('lb-overlay'))cerrarLB()}}>
        <div className="lb-box">
          <button className="lb-close" onClick={cerrarLB}>✕</button>
          <div className="lb-media">
            {lbEsp&&(()=>{
              const vs=vistasLB(lbEsp)
              const v=vs[lbVista]
              if(!v.url) return <div className="lb-nofoto"><span style={{fontSize:'2rem'}}>📷</span><span>{v.label.toUpperCase()} · PRÓXIMAMENTE</span></div>
              if(v.tipo==='video') return <video src={v.url} autoPlay loop muted playsInline style={{width:'100%',height:'100%',objectFit:'contain'}}/>
              return <img src={v.url} alt={v.label} style={{width:'100%',height:'100%',objectFit:'contain'}}/>
            })()}
          </div>
          <div className="lb-nav">
            <button className="lb-arr" onClick={()=>setLbVista(v=>Math.max(0,v-1))} disabled={lbVista===0}>&#8592;</button>
            <div className="lb-tabs">
              {lbEsp&&vistasLB(lbEsp).map((v,i)=>(
                <button key={i} className={`lb-tab${lbVista===i?' lb-on':''}`} onClick={()=>setLbVista(i)}>{v.label}</button>
              ))}
            </div>
            <button className="lb-arr" onClick={()=>setLbVista(v=>Math.min(3,v+1))} disabled={lbVista===3}>&#8594;</button>
          </div>
          {lbEsp&&<div style={{textAlign:'center',fontSize:'.75rem',fontStyle:'italic',color:'#E8C97A',marginBottom:4}}>{lbEsp.n}</div>}
          <div className="lb-hint">Toca fuera para cerrar</div>
        </div>
      </div>
    </div>
  )
              ...
              {pagEsp.map((e,i)=>(
                <button key={i} onClick={()=>setSel(e)} className="esp-card">
                  <div style={{width:'100%',height:75,background:'rgba(201,168,76,0.06)',borderRadius:5,marginBottom:6,display:'flex',alignItems:'center',justifyContent:'center',overflow:'hidden'}}>
                    {e.foto?<img src={e.foto} style={{width:'100%',height:'100%',objectFit:'cover'}} onClick={(ev)=>{ev.stopPropagation();abrirLB(e)}}/>:(
