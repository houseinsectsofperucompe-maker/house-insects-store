    ...
    <div style={{minHeight:'100vh',background:'#1A1209',fontFamily:'Georgia,serif',padding:'32px 16px'}}>
      ...
      <div style={{maxWidth:1200,margin:'0 auto'}}>
        ...
        <div style={{textAlign:'center',marginBottom:14}}>
          <input value={q} onChange={e=>{setQ(e.target.value);setPag(1)}} placeholder={useT(`Buscar en ${fid}...`)} style={{width:'100%',maxWidth:340,padding:'8px 14px',background:'#2A2010',color:'#E8C97A',border:'1px solid #C9A84C',borderRadius:8,fontSize:'.8rem',outline:'none'}}/>
        </div>
        {filtrados.length>0?(
          <>
            <div style={{color:'rgba(232,201,122,0.4)',fontSize:'.7rem',textAlign:'center',marginBottom:10}}>
              {useT('Mostrando')} {(pag-1)*POR_PAG+1}–{Math.min(pag*POR_PAG,filtrados.length)} {useT('de')} {filtrados.length} {useT('especies')} · {useT('Página')} {pag} {useT('de')} {totalPag}
            </div>
