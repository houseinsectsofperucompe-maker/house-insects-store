...
`}</style>
  ...
  return (
    <div style={{minHeight:'100vh',background:'#1A1209',fontFamily:'Georgia,serif',padding:'32px 16px'}}>
      ...
      {showQ&&<PopupCalidad onClose={()=>setShowQ(false)}/>}
      {showA&&<PopupAbrev onClose={()=>setShowA(false)}/>}
      <a href="/" className="inicio-btn" style={{color:'#C9A84C',fontSize:'1.2rem',fontWeight:700,textDecoration:'none',display:'inline-block',marginBottom:20,padding:'10px 20px',background:'rgba(201,168,76,0.15)',borderRadius:8,border:'1px solid rgba(201,168,76,0.4)'}}>← Inicio / Home</a>
