...
type Fam={id:string,nm:string,e:string[],sub:FamRaw[]}
...
export default function NocturnasPage(){
  ...
  useEffect(()=>{
    r.get<FamRaw[]>('catalogo:familias').then(d=>{
      if(!d) return
      const noc=d.filter(f=>f.orden==='Moths Nocturnas')
      // familias principales (sin padre)
      ...
      {paso===1&&(
        <div style={{maxWidth:900,margin:'0 auto'}}>
          ...
          <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fill,minmax(130px,1fr))',gap:8,marginBottom:'1.5rem'}}>
            {familias.map(f=>(
              <button key={f.id} onClick={()=>{setFamSel(f.id);setSubSel('')}}
                ...
                {f.nm}
                {f.sub.length>0&&(
                  <span style={{display:'block',fontSize:'0.6rem',color:'rgba(201,168,76,0.5)',marginTop:2}}>
                    {f.sub.length} subfamilias
                  </span>
                )}
              </button>
            ))}
          ...
          <div style={{border:`1px solid ${BD}`,borderRadius:8,padding:'1rem',marginBottom:'1.5rem',minHeight:60}}>
            {(subSel?famActual?.sub.find(s=>s.id===subSel)?.e:famActual?.e)?.length?(
              <div style={{display:'flex',flexWrap:'wrap',gap:6}}>
                {(subSel?famActual?.sub.find(s=>s.id===subSel)?.e:famActual?.e)!.map(e=>(
                  <span key={e} style={{padding:'5px 12px',border:`1px solid ${BD}`,borderRadius:20,fontSize:'0.72rem'}}>{e}</span>
                ))}
              </div>
            ):(
