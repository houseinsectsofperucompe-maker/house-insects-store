...
const MARCOS=[
  ...
  { id:'mixto', nm:'Cuadro Mixto (varios insectos)', color:'#5a3e1b', fondo:'#fff' },
]
...
export default function ColeopteraPage(){
  ...
  const [insecto,setInsecto]=useState<Esp|null>(null)
  ...
  const famActual=fams.find(f=>f.id===famSel)||fams[0]

  const handleTab=(t:'Coleoptera'|'Arthropoda')=>{
    setTab(t);setPaso(1);setInsecto(null)
    ...
    <div style={{minHeight:'100vh',background:BG,color:G,fontFamily:'Georgia,serif'}}>
      ...
      <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',padding:'12px 24px',borderBottom:'1px solid rgba(201,168,76,0.2)',background:'rgba(26,18,9,0.97)'}}>
        <div style={{display:'flex',gap:8}}>
          ...
          <a href='/catalogo/coleoptera' style={{display:'flex',alignItems:'center',gap:6,padding:'7px 14px',background:'rgba(201,168,76,0.08)',border:'1px solid rgba(201,168,76,0.3)',color:G,borderRadius:8,textDecoration:'none',fontFamily:'Georgia,serif',fontSize:'.75rem'}}>
            ← Catálogo
          </a>
        </div>
        <div style={{textAlign:'center'}}>
          <h1 style={{fontSize:'1.1rem',letterSpacing:'0.1em',margin:0,color:G}}>Cuadros de Insectos</h1>
          <p style={{fontSize:'0.65rem',color:'rgba(201,168,76,0.5)',margin:0}}>Coleoptera & Arthropoda - Shadow Box 3D - CITES/SERFOR</p>
        </div>
      ...
      <div style={{display:'flex',justifyContent:'center',alignItems:'center',gap:16,padding:'1rem',marginBottom:'1rem'}}>
        {[{n:1,nm:'Escoge tu Insecto'},{n:2,nm:'Escoge tu Marco'},{n:3,nm:'Tu Cuadro'}].map((p,i)=>(
            ...
            {loading?(
              ...
              <>
                <div style={{display:'flex',flexWrap:'wrap' as const,gap:8,marginBottom:16,justifyContent:'center'}}>
                  {fams.map(f=>(
                    <button key={f.id} onClick={()=>{setFamSel(f.id);setInsecto(null)}}
                ...
                <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fill,minmax(180px,1fr))',gap:8,marginBottom:20}}>
                  {famActual?.e?.length>0?famActual.e.map((e,i)=>(
                    <button key={i} onClick={()=>setInsecto(e)}
                      style={{background:insecto?.n===e.n?'rgba(201,168,76,0.15)':'rgba(201,168,76,0.04)',
                        border:`1px solid ${insecto?.n===e.n?G:BD}`,borderRadius:9,padding:10,
                        cursor:'pointer',textAlign:'left' as const,fontFamily:'Georgia,serif'}}>
        ...
        {paso===2&&insecto&&(
          <div>
            <div style={{display:'flex',alignItems:'center',gap:12,marginBottom:24,padding:16,
              ...
              <div>
                <p style={{color:G,fontStyle:'italic',margin:0,fontSize:'.9rem'}}>{insecto.n}</p>
                {insecto.p&&<p style={{color:'rgba(201,168,76,0.6)',margin:0,fontSize:'.75rem'}}>${insecto.p} USD</p>}
              </div>
                      ...
                      <div style={{flex:1,width:'100%',background:'#f8f8f8',display:'flex',alignItems:'center',justifyContent:'center',padding:16,position:'relative' as const}}>
                        <div style={{width:'100%',height:'100%',border:'10px solid #e8dcc8',background:'#fff',display:'flex',flexDirection:'column' as const,alignItems:'center',justifyContent:'center',position:'relative' as const,padding:8}}>
                          <div style={{flex:1,width:'100%',background:'#ffffff',display:'flex',alignItems:'center',justifyContent:'center'}}>
                            {insecto.foto?<img src={insecto.foto} style={{width:'85%',height:'85%',objectFit:'contain'}}/>:<span style={{color:'#999',fontSize:'.8rem'}}>Sin foto</span>}
                          </div>
                          <div style={{textAlign:'center' as const,padding:'6px 0 2px'}}>
                            <p style={{fontSize:'.55rem',fontStyle:'italic',color:'#333',margin:0}}>{insecto.n}</p>
              ...
              <div style={{minWidth:260,maxWidth:320}}>
                ...
                <div style={{background:'rgba(201,168,76,0.06)',border:`1px solid ${BD}`,borderRadius:8,padding:20,marginBottom:16}}>
                  <div style={{marginBottom:12,paddingBottom:12,borderBottom:`1px solid ${BD}`}}>
                    <p style={{color:'rgba(201,168,76,0.5)',fontSize:'.65rem',margin:'0 0 4px'}}>INSECTO</p>
                    <p style={{color:G,fontSize:'.85rem',fontStyle:'italic',margin:0}}>{insecto.n}</p>
                  </div>
                  <div style={{marginBottom:12,paddingBottom:12,borderBottom:`1px solid ${BD}`}}>
