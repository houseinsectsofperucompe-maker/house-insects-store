        ...
        {seleccionado&&banner&&(
          <div className="fade-up" style={{background:'rgba(201,168,76,0.06)',border:'1px solid rgba(201,168,76,0.2)',borderRadius:12,padding:24,textAlign:'center',marginBottom:32}}>
            ...
            <div style={{display:'flex',gap:10,justifyContent:'center',flexWrap:'wrap'}}>
              <button onClick={()=>{if(banner){addItem({id: String(Date.now()), nombre: banner.nombre+' (Banner '+PLANES.find(p=>p.id===plan)?.label+')', n: banner.nombre+' (Banner '+PLANES.find(p=>p.id===plan)?.label+')', p: precio, qty: 1, rubro: 'Publicidad', foto: '', familia: ''});setOk(true);setShowCarrito(true);setTimeout(()=>setOk(false),2000)}}} style={{background:ok?'#5DBB63':'#C9A84C',color:'#1A1209',padding:'12px 28px',borderRadius:4,fontWeight:700,fontSize:'.85rem',border:'none',cursor:'pointer',marginBottom:8,display:'block',width:'100%'}}>{ok?'✅ Agregado':'🛒 Agregar al carrito'}</button>
              ...
              <button onClick={()=>{if(banner){addItem({id: String(Date.now()), nombre: banner.nombre+' Banner '+plan, n: banner.nombre+' Banner '+plan, p: precio, qty: 1, rubro: 'Publicidad', foto: '', familia: ''});setShowCarrito(true)}}} style={{background:'#C9A84C',color:'#1A1209',padding:'12px 28px',borderRadius:4,fontWeight:700,fontSize:'.85rem',border:'none',cursor:'pointer',width:'100%',marginTop:8}}>🛒 Agregar al carrito</button>
            </div>
          </div>
        )}
