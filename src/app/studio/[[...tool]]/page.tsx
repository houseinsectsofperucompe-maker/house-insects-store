...
const CATALOGOS = [
  ...
  { id:3, nombre:'Joyería Natural', descripcion:'Oro · Plata · Alas naturales · Artesanal · Pieza única', video:'https://res.cloudinary.com/dv3mvukmq/video/upload/v1778854438/WhatsApp_Video_2026-05-15_at_08.40.48_sg6ybr.mp4', imagen:'https://res.cloudinary.com/dv3mvukmq/image/upload/Joyerias_y_cuadros_en_oro_Y_PLATA_y67in5', colorFondo:'rgba(30,20,0,0.85)' },
  { id:4, nombre:'Rarezas & Gynandromorphs', descripcion:'Piezas únicas · Aberraciones · Gynandromorphs · Colección élite', video:'https://res.cloudinary.com/dv3mvukmq/video/upload/v1778870094/house_insects_peru_intro_ujjmrv.mp4', imagen:'https://res.cloudinary.com/dv3mvukmq/image/upload/rarezas_y_gyanadrmophos_yeno1o', colorFondo:'rgba(20,10,30,0.85)' },
  { id:5, nombre:'Artesanías & Cúpulas', descripcion:'Cuadros · Cúpulas · Resinas · Arte natural amazónico', video:'https://res.cloudinary.com/dv3mvukmq/video/upload/v1778854438/WhatsApp_Video_2026-05-15_at_08.40.39_wnlddk.mp4', imagen:'https://res.cloudinary.com/dv3mvukmq/image/upload/ARTEZANIAS_DE_COPULAS_vsiioa', colorFondo:'rgba(30,15,5,0.85)' },
    ...
    return (
      <div style={{ minHeight:'100vh', background:'#000', position:'relative', overflow:'hidden', fontFamily:'Georgia, serif' }}>
        ...
        <div style={{ position:'relative', zIndex:10, minHeight:'100vh', display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center', textAlign:'center', padding:'40px' }}>
          <img src={cat.imagen} alt={cat.nombre} style={{ width:140, height:140, marginBottom:20, borderRadius:'50%', border:'2px solid #C9A84C', objectFit:'cover' }} />
          <h1 style={{ fontSize:'clamp(1.8rem,5vw,3.5rem)', fontWeight:400, color:'#E8C97A', letterSpacing:'.08em', marginBottom:12, textShadow:'0 2px 12px rgba(0,0,0,0.8)' }}>{cat.nombre}</h1>
          <p style={{ fontSize:'clamp(.9rem,2vw,1.1rem)', color:'rgba(255,255,255,0.8)', marginBottom:32, maxWidth:500, lineHeight:1.8 }}>{cat.descripcion}</p>
          ...
          <a href="mailto:houseinsectsofperu.com.pe@gmail.com" style={{ color:'#C9A84C', fontSize:'.85rem', marginBottom:20, textDecoration:'none' }}>✉️ houseinsectsofperu.com.pe@gmail.com</a>
          <br/>
          <button onClick={() => setActivo(null)} style={{ background:'transparent', border:'1px solid rgba(255,255,255,0.3)', color:'rgba(255,255,255,0.6)', padding:'10px 24px', borderRadius:4, fontSize:'.8rem', cursor:'pointer', marginTop:12 }}>← Volver al inicio</button>
        </div>
      </div>
    )
  }
  ...
  return (
    <div style={{ minHeight:'100vh', background:'#1A1209', fontFamily:'Georgia, serif' }}>
      <div style={{ display:'flex', flexDirection:'column', alignItems:'center', padding:'50px 20px 30px', textAlign:'center' }}>
        ...
        <p style={{ fontSize:'.75rem', color:'rgba(232,201,122,0.3)', letterSpacing:'.08em', marginBottom:30 }}>RUC 20447397804 · LEY AMAZÓNIA N°27037 · CITES · SERFOR</p>
        ...
        {CATALOGOS.map(c => (
          <button key={c.id} onClick={() => setActivo(c.id)} style={{ position:'relative', aspectRatio:'1/1', background:'#0A0A0A', border:'none', cursor:'pointer', overflow:'hidden' }}>
            ...
            <div style={{ position:'absolute', bottom:0, left:0, right:0, padding:'20px', textAlign:'center' }}>
              <h3 style={{ color:'#E8C97A', fontSize:'clamp(.8rem,2vw,.95rem)', fontWeight:400, letterSpacing:'.06em', marginBottom:6, textShadow:'0 2px 8px rgba(0,0,0,0.9)' }}>{c.nombre}</h3>
      ...
      <div style={{ textAlign:'center', padding:'20px', borderTop:'1px solid rgba(201,168,76,0.15)' }}>
        <p style={{ color:'rgba(232,201,122,0.25)', fontSize:'.68rem', letterSpacing:'.08em' }}>© 2026 HOUSE INSECTS OF PERU E.I.R.L. · TINGO MARÍA, PERÚ · EXPORTACIÓN MUNDIAL</p>
      </div>
    </div>
  )
}