...
export async function GET() {
  try {
    if (!PAGE_ID || !PAGE_ACCESS_TOKEN) return NextResponse.json({ error: 'Variables no configuradas' }, { status: 500 });
    const res = await fetch(`https://graph.facebook.com/v25.0/${PAGE_ID}?fields=name,id&access_token=${PAGE_ACCESS_TOKEN}`);
    const data = await res.json();
    if (!res.ok || data.error) return NextResponse.json({ error: data.error?.message }, { status: 400 });
